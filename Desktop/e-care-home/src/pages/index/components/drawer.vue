...
const activeSessionId = computed(() => chatStore.activeSession?.sessionId)
...
async function handleClickSession(sessionId: string) {
  if (!selecting.value) {
    await chatStore.getSessionDetail(sessionId)
...
<template>
  <up-popup v-model:show="appStore.showDrawer" class="absolute" mode="left">
    <view class="drawer-container">
      <view class="mb-8 flex items-center gap-4">
        <up-avatar size="50" src="@/static/images/avatar.png" />
        <view @click="goToPage('profile')">
          <view class="text-lg font-bold">
            路小雨
          </view>
          <text class="text-xs text-gray-400">点击修改个人资料</text>
        </view>
        <up-icon name="setting" size="24" class="ml-auto" color="#999" @click="goToPage('setting')" />
      </view>
          ...
          <view v-for="sessions, key in chatStore.sessionListByLabel" :key="key">
            ...
            <view class="flex flex-col gap-8px">
              <view v-for="session in sessions" :key="session.sessionId"
                ...
                :class="{ 'is-longpress': currentSession?.sessionId === session.sessionId, 'is-active': activeSessionId === session.sessionId }"
                @longpress="handleLongPress(session, $event)" @click="handleClickSession(session.sessionId)">
