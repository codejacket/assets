...
<template>
  <view>
    <up-popup v-model:show="isFullscreen" class="absolute" round="0">
      <view class="chat-editor-fullscreen">
        <textarea v-model="modelValue" :maxlength="-1" auto-height />
      </view>
    </up-popup>
    <view v-if="!isFullscreen" class="chat-editor">
      ...
      <view class="h-full flex items-end">
        ...
        <view v-if="isVoiceMode" class="chat-editor-voice-label" :class="{ 'is-disabled': disabled }"
          @touchstart="handleTouchstart" @touchend="handleTouchend" @touchcanel="handleTouchend"
        ...
        <view class="flex flex-col-reverse justify-between" style="height: stretch;">
          <up-icon v-if="modelValue" class="p-6px" :color="disabled ? '#ccc' : 'blue'" name="checkmark-circle-fill"
            size="24" @click="handleSend" />
    ...
    <transition>
      <view v-if="showMoreTools && !isFullscreen" class="more-tools-container">
        <up-grid col="4">
          <up-grid-item>
